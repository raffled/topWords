...
runDist <- function(input.file, n.clades, n.dist.values = 1, metric = "euclidean",
                  p = 2, linkage = "ave", is.tab.sep=TRUE, dataset.name = "dist-output", d.metric = 1)
  {
    ...
    ##If one or both of the last two rows of gen.data contain NA values for the word counts,
    ##they contain the "metadata" from scrubber and/or divitext.  Re-copy gen.data to itself
    ##omitting these rows so the program can actually run.
