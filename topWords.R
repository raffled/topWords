...
####Behavior
## This function will create an output directory within your working
## directory named from the dataset.name parameter.  Within that
## directory it will create text files summarizing the results, a
## dendrogram showing which chunks are in which clade after cutting,
## and graphs of the relative frequencies across clades for all the
## words within the top n distance values specified.  Upon completion
## the function will return to the original working directory.
...
##  dataset.name : used as a label for various outputs (e.g. graph names and labels)
##  d.metric : is the metric to use for analysis.  All metrics will have a numeric code:
##           : 1 - Kruskal-Wallis (Default)
##           : 2 - Beta-Binomial Anova-F
##           : 3 - Absolute Distance
##  run.betabin.par : lets user choose to run the betabin function on multiple CPUs.
##                  : (need to have "multicore" package installed.  Default is FALSE
##  num.cores : lets user choose number of cores to use for parallel computation of
##            : betabin anova.  Default is 2.  If betabin is not being run or is being
##            : run sequentially, this parameter doesn't matter.

####Return Value:
## no return value

#### Warnings:
## If the program is stopped before completion, the working directory will in the output
## directory.  If debugging, it may be wise to keep track of the working directory so you
## don't get nested in an inception-like directory structure.
...
topWords <- function(input.file, n.clades, n.dist.values = 1, metric = "euclidean",
                  p = 2, linkage = "ave", is.tab.sep=TRUE,
                      dataset.name = "dist-output", d.metric = 1,
                      run.betabin.par=FALSE, num.cores=2){
        ...
        for(j in 1:length(topwords.dist[[i]]))
          {
            ...
            #close pdf device(s) so R doesn't break itself from too many open devices
            graphics.off()
          }
...
##for more useful printing.  Only works on 1-20, anything more will
##get 21TH, etc for the time being.
getOrder <- function(n){
  order <- NULL
...
getBetaBinAnova <- function(number.words, fdoc.collection, gen.counts,
    ...
    #start system time, as this can take a long time.
    start.t <- Sys.time()
    ...
    #process sequentially
        ...
        {
            L1 <- attributes(
                      m1 <- betabin(cbind(gen.counts[,j],
                                          rsums - gen.counts[,j])~fdoc.collection - 1,
                                    ~1, data=gen.counts,
                                    control = list(maxit = 10, reltol = .001),
                                    fixpar=list(n.clades+1, 0)))$logL
            L0 <- attributes(
                m0 <- betabin(cbind(gen.counts[,j],
                                    rsums - gen.counts[,j])~1,
                              ~1, data=gen.counts,
                              control = list(maxit = 10, reltol = .001),
                              fixpar=list(2, 0)))$logL
            ...
            ##don't want to slow things down too much or spam output,
            ##so only print on even values
            if(j %% 5 == 0){
                print(paste(round(percent.complete, digits=1), "% complete | ",
                            (format(Sys.time() - start.t, usetz = TRUE)), sep=""))
                #gc() still stable w/o this, and save boatloads of time
            }
        }
    }
...
lineColor <- function(x, colorOfNodes){
    ...
    x # override lazy-eval used by R
}
...
generateLineColorList <- function(x, mergeTableRow, specialLabels,
    ...
    if(x$merge[mergeTableRow,1] < 0) #if the left node is a chunk determine the chunk's color
        {
            ...
            leftColor <- getColor(x$labels[-x$merge[mergeTableRow,1]],
	...
	{
		rightColor <- getColor(x$labels[-x$merge[mergeTableRow,2]],
                                       specialLabels=specialLabels,
                                       metaTable = metaTable, colors) #the color of the chunk
	...
	#the colors found need to be put together in the proper order.
        #The current clade has one node for each of it's childern which contains a
        #clade instead of just a chunk.
	#Those nodes need to be given the color of the current clade, but only if they exist.
        #These nodes will appear in the list of colors before all the colors for the nodes in the respective
	#subclades
                          ...
                          dataset.name, fdoc.collection, ...)
{
    ...
    pdf(file=paste(outputFilename, ".pdf", sep=""),paper = "a4", width=11, height=8.5)
